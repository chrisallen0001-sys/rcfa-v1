...
interface AddRootCauseFormProps {
  rcfaId: string;
}

export default function AddRootCauseForm({ rcfaId }: AddRootCauseFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/rcfa/${rcfaId}/root-causes/finals`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error ?? "Failed to add root cause");
      }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to add root cause"
      );
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded-md border border-zinc-200 p-4 dark:border-zinc-800">
      <div>
        <label htmlFor="causeText" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
      ...
      <div>
        <label htmlFor="evidenceSummary" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
          Evidence Summary
        </label>
        <textarea
          id="evidenceSummary"
