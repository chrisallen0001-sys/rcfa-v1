...
function AletheiaLogo() {
  ...
  // The original SVG viewBox is "0 0 210 297" but the meaningful content
  // spans roughly x:70-145, y:158-190. We use a trimmed viewBox for the logo area.
  return (
    <Svg width={100} height={28} viewBox="68 158 80 32">
            ...
            {rcfa.actionItems.map((ai, idx) => (
              <View key={ai.id} wrap={false}>
                ...
                {(ai.actionDescription || ai.completionNotes) && (
                  <View style={{ paddingHorizontal: 6, paddingVertical: 4, paddingLeft: 30, backgroundColor: idx % 2 === 1 ? COLORS.bgStripe : COLORS.white }}>
                    {ai.actionDescription && (
                      <View style={{ marginBottom: 2 }}>
                        <Text style={{ fontSize: 7, fontFamily: "Helvetica-Bold", color: COLORS.secondary }}>
                          Description:
                        </Text>
